...
if ! ldconfig -p 2>/dev/null | grep -q "libcudart.so.12"; then
  ...
  echo "CUDA runtime not found. Installing..."

  $SUDO apt-get install -y cuda-cudart-12-9 || \
  $SUDO apt-get install -y cuda-cudart-12-8 || \
  $SUDO apt-get install -y cuda-cudart-12-6 || \
  $SUDO apt-get install -y cuda-cudart-12-2 || \
  $SUDO apt-get install -y libcudart12 || true
